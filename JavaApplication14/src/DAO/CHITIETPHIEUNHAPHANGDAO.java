...
public class CHITIETPHIEUNHAPHANGDAO {
    ...
    public boolean sua(CHITIETPHIEUNHAPHANGDTO dto) throws SQLException{
        String qry="update chitietphieunhaphang set DONGIA='"+dto.getDONGIA()+"',SOLUONG='"+dto.getSOLUONG()+"',THANHTIEN='"+dto.getTHANHTIEN()+"' where MAPNH='"+dto.getMAPNH()+"' AND MASP='"+dto.getMASP()+"'";    
        Connect con=new Connect();
    ...
    public boolean Xoa(CHITIETPHIEUNHAPHANGDTO dto) throws Exception{
        ...
        String query="DELETE FROM chitietphieunhaphang WHERE MAPNH='"+dto.getMAPNH()+"' AND MASP='"+dto.getMASP()+"'";
        Connect conn=new Connect();
        int a=conn.excuteUpdate(query);
        if(a>0){
            ...
            JOptionPane.showMessageDialog(null,"Xóa thất bại db");
            return false;
        }
    }
